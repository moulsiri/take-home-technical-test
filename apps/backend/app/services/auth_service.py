...
async def handle_login_payload(user: User, response: Response, db: AsyncSession):
...
async def register_new_user(user_data: UserCreate, response: Response, db: AsyncSession):
    ...
    return await handle_login_payload(new_user, response, db)

async def authenticate_user(user_data: UserLogin, response: Response, db: AsyncSession):
    result = await db.execute(select(User).where(User.email == user_data.email))
    user = result.scalar_one_or_none()
    if not user or not verify_password(user_data.password, user.password_hash):
        raise HTTPException(status_code=401, detail="Invalid email or password")
        
    return await handle_login_payload(user, response, db)

async def refresh_user_session(ref_token: str | None, response: Response, db: AsyncSession):
    if not ref_token:
...
async def logout_user_session(ref_token: str | None, response: Response, db: AsyncSession):
    if ref_token:
        try:
            payload = decode_refresh_token(ref_token)
...
async def trigger_forgot_password(data: ForgotPassword, db: AsyncSession):
...
async def execute_reset_password(data: ResetPassword, db: AsyncSession):
